...
AHit_Checker *ABall::Hit_Chekers[] = {};
//----------------------------------------------------------------------------------------------------
ABall::ABall()
    : Ball_State(EBS_Normal), Ball_Pen(0), Ball_Brush(0), Center_X_Pos(0.0), Center_Y_Pos(Start_Ball_Y_Pos),
...
void ABall::Move()
{
    ...
    while (Rest_Distance >= AsConfig::Moving_Step_Size)
    {
        ...
        if (!got_hit)
        {
            //Ball directed if he don't triggerred with other objects
...
void ABall::Set_For_Test()
{
    ...
    Set_State(EBS_Normal,70 + Test_Iteration, 100);
...
{
    if (Ball_Direction > M_PI_2 && Ball_Direction < M_PI_2 + M_PI)
        return true;
    else
        return false;
}
...
//----------------------------------------------------------------------------------------------------
void ABall::Add_Hit_Cheker(AHit_Checker* hit_checker)
{
    if(Hit_Checkers_Count >= sizeof(Hit_Chekers) / sizeof(Hit_Chekers[0]))
        return;

    Hit_Chekers[Hit_Checkers_Count++] = hit_checker;
}
//----------------------------------------------------------------------------------------------------
