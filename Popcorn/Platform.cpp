...
//----------------------------------------------------------------------------------------------------
AsPlatform::AsPlatform()
    : X_Pos(AsConfig::Border_X_Offset), X_Step(AsConfig::Global_Scale * 2), Platform_State(EPS_Normal), 
    ...
    Width(Normal_Width), Platform_Rect{}, Prev_Platform_Rect{}, Platform_Circle_Pen(0), Platform_Circle_Brush(0),
    Platform_Inner_Pen(0), Platform_Inner_Brush(0), Highlight_Pen(0), Highlight_Pen_Color(255, 255, 255),
...
bool AsPlatform::Check_Hit(double next_x_pos, double next_y_pos, ABall* ball)
{
    ...
    inner_low_y = AsConfig::Platform_Y_Pos + Height - 1;
    inner_left_x = X_Pos + Circle_Size - 1;
    inner_right_x = X_Pos + Width - (Circle_Size - 1);

    if (Reflect_On_Circle(next_x_pos, next_y_pos, 0.0, ball))
        return true;  //Reflection from left circle of platform

    if (Reflect_On_Circle(next_x_pos, next_y_pos, Width - Circle_Size ,ball))
        return true;  //Reflection from right circle of platform

    //Cheking reflection of central part of platform
    if (ball->Is_Moving_Up())
        inner_y = inner_low_y;  //low
    else
        inner_y = inner_top_y;  //top
...
void AsPlatform::Draw_Normal_State(HDC hdc, RECT& paint_area)
{
    int i, j;
    int offset = 0;
    //Drowing platform in normal state
    int x = X_Pos;
...
{
    //Drowing platform in meltdown state
    int i, j;
    int x, y;
    int y_offset;
    int stroke_len;
    ...
    for (i = 0; i < Normal_Platform_Image_Width; i++)
    {
        ...
        //Draw sequence of vertical stroces different color
    ...
    if (moved_columns_count == 0)
        Platform_State = EPS_Missing;  // all platforme Beyond a frame
}
//----------------------------------------------------------------------------------------------------
...
bool AsPlatform::Reflect_On_Circle(double next_x_pos, double next_y_pos, double platform_ball_x_offset, ABall* ball)
{
    ...
    {
        //ball toched sphere side of platform
        beta = atan2(-dy, dx);
...
{
    //calculete lenght of next vertical stroke

    int i;
    int offset = y * Normal_Platform_Image_Width + x;  //Position on array Normal_Platform_Image, corresponding to the offset(x, y)
    int color;
    ...
    for (i = y; i < Normal_Platform_Image_Height; i++)
    {
        ...
        offset += Normal_Platform_Image_Width; // Turn down on next string
    }
